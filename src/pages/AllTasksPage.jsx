...
export default function AllTasksPage({ deliverables, currentRole, currentUserId, onCycleStatus, onDelete }) {
    ...
    <div>
      ...
      <div className="tasks-stats">
        <div className="task-stat">
          <div className="task-stat-icon" style={{ background: 'rgba(245,158,11,0.1)', color: 'var(--warn)' }}><svg><use href="#icon-clock" /></svg></div>
          <div><div className="task-stat-num">{pending}</div><div className="task-stat-lbl">Pending / Approval</div></div>
        </div>
        <div className="task-stat">
          <div className="task-stat-icon" style={{ background: 'rgba(30,86,217,0.09)', color: 'var(--accent)' }}><svg><use href="#icon-check-list" /></svg></div>
          <div><div className="task-stat-num">{open}</div><div className="task-stat-lbl">Open Deliverables</div></div>
        </div>
        <div className="task-stat">
          <div className="task-stat-icon" style={{ background: 'rgba(0,201,167,0.09)', color: 'var(--accent2)' }}><svg><use href="#icon-check-circle" /></svg></div>
      ...
      <div className="tasks-toolbar">
        <div className="search-wrap">
          ...
          <input type="text" className="search-input" placeholder="Search tasks, deliverables, claims..." onChange={e => setSearch(e.target.value.toLowerCase())} />
        </div>
        ...
        <select className="filter-select" onChange={e => setClaimFilter(e.target.value)}>
          <option value="all">All Claims</option>
          {CLAIMS.map(c => <option key={c.id} value={c.id}>GNC #{c.gnc} · {c.name}</option>)}
        </select>
      </div>
